        	...
        	try {
    			if(jwtTokenProvider.validateToken(token)) {
        			...
        			if(jwtTokenProvider.isBlacklistToken(token)) {
        				log.warn("[JWT] Token is blacklist! {}", token);
        				tokenExpired((HttpServletRequest)request, (HttpServletResponse)response, token, ApiResultError.NO_AUTH_TOKEN_BLACKLIST);
        				return;
        			}

        			if(!jwtTokenProvider.isAllowAccessToken(token)) {
        				log.warn("[JWT] Token is not allowed! {}", token);
        				tokenExpired((HttpServletRequest)request, (HttpServletResponse)response, token, ApiResultError.NO_AUTH_TOKEN_BLACKLIST);
        				return;
        			}
