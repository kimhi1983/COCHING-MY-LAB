...
import { getUserBaseUrl } from '@/api/coching/comm/common';
...
export default {
  ...
  computed: {
    ...
    isMainPage(){
      return this.$route.name == "coching-raw-main"
    },
    partnerInfo(){
  ...
  methods: {  
    ...
    //사용자 BASE URL 가져오기
    async ermLoadUserBaseUrl(){
      const _vm = this;

      try{
        // 기업 정보
        const resUrlInfo = await getUserBaseUrl();
        if("0000" == resUrlInfo.resultCode) {
          const {resultData} = resUrlInfo;
          const reqUrl = resultData;

          _vm.$store.dispatch('coching/setUserBaseUrlData', reqUrl);
        }
