...
public class SnsLoginController extends BaseLoginController{
	...
	private static final String REDIRECT_USER_SNS_LOGIN_URL = "redirect:/coching-pn/snsLogin";

	//@Value("${server.base.url:https://www.coching.co.kr}" )
	@Value("#{'${server.base.url:https://partner.coching.co.kr}'.replaceAll('\\s+', '').split(',')}")
	...
	public static String getMessageForWithdrawLimit(MemberVO membVo) {
		...
		String retMsg = null;
//		if(membVo != null && membVo.getWithdrawDate() != null) {
//			mftmsg +="\n탈회일자 : %s";
//
//			retMsg = String.format(mftmsg, DateUtil.SDF_DATEDOT.format(membVo.getWithdrawDate()));
//		}else {
//			retMsg = mftmsg;
//		}
