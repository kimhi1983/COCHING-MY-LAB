...
//    private ErrorMailService errorMailService;
	...
	public ModelAndView handleNotSupportedException(HttpServletRequest request, HttpServletResponse response, Exception e) throws Exception {
		...
		model.addAllObjects(new ApiResult(ApiResultError.ERROR_NOT_SUPPORTED_METHOD, LocaleType.EN));
		return model;
	}
	...
	public ModelAndView handleExpiredJwtException(HttpServletRequest request, HttpServletResponse response, Exception e) throws Exception {
		...
		model.addAllObjects(new ApiResult(ApiResultError.NO_AUTH_TOKEN_EXPIRED, LocaleType.EN));
		return model;
	}
	...
	public ModelAndView handleAccessDeniedException(HttpServletRequest request, HttpServletResponse response, Exception e) throws Exception {
		...
		model.addAllObjects(new ApiResult(ApiResultError.ERROR_ACCESS_DENIED, LocaleType.EN));
		return model;
	}
	...
	public ModelAndView handleException(HttpServletRequest request, HttpServletResponse response, Exception e) throws Exception {
		...
		log.error("Exception details - Class: {}, Message: {}, URI: {}",
				e.getClass().getName(), e.getMessage(), request.getRequestURI(), e);

		Throwable cause = e.getCause();
		while (cause != null && cause.getCause() != null) {
			cause = cause.getCause(); // 가장 안쪽 cause 까지 내려가기
		}

		log.error("[MultipartException] uri={}, ip={}, lengthHeader={}, rootCause={}",
		...
		if(e instanceof HttpMessageNotReadableException) {
			...
			model.addAllObjects(new ApiResult(ApiResultError.BAD_REQUEST, LocaleType.EN));
			return model;
		}

		model.addAllObjects(new ApiResult(ApiResultError.ERROR_SERVER_ERROR, LocaleType.EN));
