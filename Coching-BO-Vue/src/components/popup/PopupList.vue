<template>
  <div class="cm-popup-wrap">

    <!-- popupMstCd:{{ popupMstCd }} -->
    ...
    <b-card no-body>
      <b-card-body>
        <b-row>
          ...
          <b-col cols="6" md="3">
            <b-form-group label="사용 여부" label-for="search-popup-useYn">
              <b-form-radio-group
                id="search-popup-useYn"
          ...
          <b-col cols="6" md="3">
            <b-form-group label="숨김 여부" label-for="search-popup-delYn">
              <b-form-radio-group
                id="search-popup-delYn"
                ...
                @change="onSearch"
                :options="[
                  {text:'전체', value: ''},{text:'표시', value: 'N'},{text:'숨김', value: 'Y'}
                ]"
              >
              </b-form-radio-group>
            </b-form-group>
          ...
          <b-col cols="12" md="6">
            <b-form-group label="팝업명" label-for="search-popup-nameL">
              <b-input-group>
                <b-form-input
                  id="search-popup-nameL"
                  v-model="listData.sc.popupNameL"
                  @keyup.enter="onSearch"
                  size="sm"
                  placeholder="팝업명을 검색하세요."
                ></b-form-input>                
                <b-input-group-append v-show="listData.sc.nameL">
...
import popupMixin from '@/components/popup/popupMixin';
import { DEF_SEARCH_OPT, POPUP_MODE } from '@/constants/popup';
...
export default {
  name: 'coching-popup-basic-list',
  mixins: [ernsUtils, ernsAgGrid, popupMixin],
  components : {
    draggable,
    AgGridVue,
    ButtonCellRenderer,
  ...
  data(){
    ...
    const AG_GRID_COLUMN_DEFS = [
      ...
      { headerName: "팝업명"      , field: "popupName", width: 300  , cellClass: 'text-left'},
      { headerName: "정렬"        , field: "sortOrd"   , width: 70  , cellClass: 'text-center'},
      { headerName: "노출기간"    , field: "popupDate" , width: 200   , cellClass: 'text-center'},
      ...
      { headerName: "숨김"      , field: "delYn"     , width: 70  //, cellClass: 'text-center'
          ,editable: true,
          cellRenderer: "ToogleButtonCellRenderer", // 렌더러 지정
          cellRendererParams: {
            field : 'delYn',
      ...
      { headerName: "설명"      , field: "popupDesc"  , width: 300   , cellClass: 'text-left'},
    ...
    return {      
      ...
      listData: {
        ...
        sc: { ...DEF_SEARCH_OPT.sc, popupMstCd : _vm.popupMstCd },
  ...
  methods : {
    ...
    onCellClicked(event){
      ...
      //상세이동
      const goDetailFileds = ["popupName", "sortOrd", "popupDate", "membId", "chngDttm", "popupDesc"];
    ...
    onCellValueChanged(params) {
      ...
      //Cell 랜더러에게 변경 noti
      const checkFields = ["_isEditRow", "popupName"];
    ...
    async onClickMultpleSave(){
      ...
      Object.entries(editCells).forEach(([rowId, editCellInfo]) => {
        ...
        params.list.push({
          popupSeq : rowNode.data.popupSeq || 0,
      ...
      const result = await _vm.$swal({
        title: '확인',
        text: `${params.list.length}개 팝업 설정을 저장 하시겠습니까?`,
      ...
      try {        
        ...
        if(result.value){
          await setPopupState(params);
          await _vm.onSearch();
          _vm.alertSuccess("팝업 설정 값이 변경되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
    ...
    async onApplyValue(gridParams){
      const _vm = this;
      const params = {
        isRegMode: (gridParams.data.popupSeq || 0) <= 0,
        popupMstCd : _vm.popupMstCd,
        popupSeq : gridParams.data.popupSeq || 0,
      ...
      const result = await _vm.$swal({
        title: '확인',
        text: `팝업 상태를 ${params.isRegMode ? '등록': '변경'} 하시겠습니까?`,
        showCancelButton: true,
      ...
      try {        
        ...
        if(result.value){
          await setPopupState({list:[params]});
          await _vm.onSearch();
          _vm.alertSuccess("팝업 상태가 변경되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
    ...
    async onDeleteRow(gridParams){
      const _vm = this;
      const params = {
        isRegMode: gridParams.data.popupSeq <= 0,
        popupMstCd : _vm.popupMstCd,
        popupSeq : gridParams.data.popupSeq || 0,        
      };
      ...
      try {        
        ...
        if(result.value){
          await delPopups({list:[params]});
          await _vm.onSearch();
          _vm.alertSuccess("팝업가 삭제 되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
    ...
    async onClickSaveOrder(){
      ...
      try {        
        ...
        if(result.value){
          await setPopupOrder(params);
          await _vm.onSearch();
          _vm.alertSuccess("팝업 순서가 저장 되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
      ...
      try {
        ...
        listData.rawList = list.map((item, index) => {
          ...
          if(item.fromDate && item.toDate){
            popupDate = item.fromDate.concat(" ~ ", item.toDate);
          }
          if(item.fromDate && !item.toDate){
            popupDate = item.fromDate;
          }
          if(!item.fromDate && item.toDate){
            popupDate = item.toDate;
          }
    ...
    getListSearchParam() {
      ...
      return {
        ...
        //팝업 아이디
        popupMstCd : searchOp.popupMstCd,
        useYn : searchOp.useYn,
        delYn : searchOp.delYn,

        popupNameL : searchOp.popupNameL,
      }
    },
