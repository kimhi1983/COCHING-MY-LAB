<template>
  <div class="cm-inqr--wrap">
          ...
          <b-row>
            <b-col md="4">
              <b-form-group label="카테고리"
                label-for="bi-form-cateCd">
                <b-form-input
                  id="bi-form-cateCd"
                  :value="getCodeName(detail.cateCd, codes.CATE)"
                  autocomplete="off"
                  readonly
                />                    
              </b-form-group>
    ...
    <!-- 답변 영역 -->
    <b-card>
      <validation-observer ref="simpleRulesReply">
        
        <b-form>
          <b-row>
            <b-col md="12">
              <b-form-group label="답변 제목"
                label-for="bi-form-replyTitle">
                <validation-provider #default="{ errors }"
                  name="'답변 제목'"
                  rules="required"
                >
                  <b-form-input
                    id="bi-form-replyTitle"
          ...
          <b-row>
            <b-col md="12">
              <b-form-group label="답변 내용"
                label-for="bi-form-replyContent">
                <validation-provider #default="{ errors }"
                  name="'답변 내용'"
                  rules="required"
                >
                  <b-form-textarea
                    id="bi-form-replyContent"
        ...
        <div class="text-center mt-4">
          <b-button
            ...
            variant="primary"
            class="mr-1"
            @click.prevent="onClickReply"
          > {{detail.isReply === 'N' ? '답변하기': '답변 수정하기'}}
          </b-button>
        </div>
      </validation-observer>
    </b-card>
...
import {getInqrBoard, setInqrBoard, delInqrBoardDetail, addInqrBoard, getInqrBoardReply} from '@/api/coching-bo/comm/board';
...
import { BOARD_MODE, DEF_INQR_BOARD_INF, DEF_FILE_INF } from '@/constants/board';
import boardMixin from '../boardMixin';



export default {
	name: 'coching-board-inqr-write',
  ...
  computed : {
	...
	data() {
      ...
      detail : {...DEF_INQR_BOARD_INF},
      fileData : JSON.parse(JSON.stringify(DEF_FILE_INF)),
      replyData : {...DEF_INQR_BOARD_INF},
      params : {},
      boardMst : {},
    }
	},
  ...
  beforeDestroy(){
    ...
    async onClickDelete(){
      ...
      try{
        ...
        //수정모드
        const res = await delInqrBoardDetail(params);
      ...
      try{
        ...
        if(_vm.isRegMode){
          ...
          */
          ...
          //수정모드
          const res = await setInqrBoard(formParam);
          _vm.loading(false);

          await _vm.alertSuccess(`게시물이 수정 되었습니다.`);
          _vm.goList(1, true);
          return;
        }
    ...
    async loadDetail(){
      ...
      try{
        ...
        const res = await getInqrBoard(params);
        console.debug(res);
        _vm.detail = {
          ...res.resultData
        }
    ...
    setParamData(param, fileData, type){
      const _vm = this;

      var orgSeq = 0;
      if(type == 'reply'){
        orgSeq = _vm.detail.boardSeq || 0;
      }
      ...
      formData.append('cateCd', _vm.detail.cateCd);
    ...
    getCodeName(val, codeList, pTraceKey) {
      let ret = "-";
      if (!codeList || codeList.length < 1 || !val) { return ret; }
      let traceKey = pTraceKey || 'value';
      const cd = codeList.find(item => item[traceKey] == val);
      if (cd) {
        return cd.label;
      }
      return "-";
    ...
    async onClickReply() {
      ...
      const isValid = await _vm.$refs.simpleRulesReply.validate();
      if(!isValid){
        return;        
      }
      ...
      const formParam = _vm.setParamData(params, _vm.fileData, 'reply');

      if(_vm.detail.isReply == 'N'){
        //답변 등록
        const res = await addInqrBoard(formParam);
        _vm.loading(false);

        await _vm.alertSuccess(`답변이 등록 되었습니다.`);
        ...
        await _vm.alertSuccess(`답변이 수정 되었습니다.`);
