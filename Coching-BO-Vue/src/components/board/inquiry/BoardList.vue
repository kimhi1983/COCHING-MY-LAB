<template>
  <div class="cm-inquiry-wrap">
  <!-- 검색조건 -->
    <b-card no-body>
      <b-card-body class="cm-inquiry-filter">
        <b-row>

          <b-col cols="12" md="3" class="mb-md-0">
            <b-form-group label="답변여부" label-for="board-inquiry-isReply">
              <v-select
                id="board-inquiry-isReply"
                v-model="listData.sc.isReply"
                :options="[{label:'답변대기', value:'N'}, {label:'답변완료', value:'Y'}]"
                :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                :class="['w-100', 'select-size-sm', 'select-placeholder-gray', { 'has-value': listData.sc.isReply }]"
                placeholder="답변여부를 선택하세요."
                @input="onSearch"
                >
          ...
          <b-col v-show="boardMst.cateCd"
            ...
            <b-form-group label="카테고리" label-for="board-inquiry-cateCd">
              <v-select
                id="board-inquiry-cateCd"
          ...
          <b-col cols="12" md='6'
            class="mb-md-0">
            <b-form-group label="검색 키워드" label-for="board-inquiry-search-keyword">
              <b-input-group>
                <b-form-input 
                  id="board-inquiry-search-keyword"
...
import {getInprBoardList, delBoardDetails} from '@/api/coching-bo/comm/board';
...
export default {
  ...
  data(){
    ...
    const AG_GRID_COLUMN_DEFS = [
      ...
      { headerName: "답변여부", field: "isReply", width: 80, cellClass: 'flex-center'
        ,valueFormatter: (params) => {
          return params.value === 'Y' ? '답변완료' : '답변대기';
        }
        , cellClassRules: {
          'text-success' : (params) => params.value === 'Y',
          'text-danger' : (params) => params.value === 'N',
        }
      },
      ...
      /*
      {
        headerName: "삭제"      , field: "delButton"  , width: 100  , cellClass: 'text-center',
  ...
  methods : {
   ...
   //Cell 클릭 이벤트
    onCellClicked(event){
      ...
      //상세이동
      const goDetailFileds = ["cateCd", "isReply", "title", "regName", "regId", "chngDttm"];
    ...
    getListSearchParam() {
      ...
      return {
        ...
        isReply : searchOp.isReply?.value || '',
      }
    },
