      ...
      <validation-observer ref="simpleRules">
        <b-form>
          <b-row>
            ...
            <b-col md="6"
              v-if="codes.CATE.length > 0"
            >
              <b-form-group label="카테고리" label-for="bi-form-cate">
                <validation-provider #default="{ errors }"
                  name="'카테고리'"
                  :rules="codes.CATE.length > 0 ? 'required' : ''"
                >
                  <v-select
                    id="bi-form-cate"
                    v-model="detail.cateCd"
            ...
            <!--file-wrap-->
            <b-col md="12"
              v-show="boardMstId=='BD_NT_001' || boardMstId=='BD_NT_002'
                    || boardMstId=='BD_WN_001' || boardMstId=='BD_WN_002'"
            >
...
import {getBoardDetail, setBoardDetail, addBoardDetail, delBoardDetail, uploadFile} from '@/api/coching-bo/comm/board';

import { required } from '@validations';
import Ripple from 'vue-ripple-directive';
import vSelect from 'vue-select';
import TextQuillEditor from '@/components/common/TextQuillEditor.vue'
...
export default {
	name: 'coching-board-notice-write',
	mixins: [ernsUtils, boardMixin],
	components : {
    vSelect, required, BoardFile, TextQuillEditor
  },
  directives: {
    Ripple
  },
  ...
  beforeDestroy(){
    ...
    setAllowedFileExtensions() {
      ...
      if( _vm.boardMstId == 'BD_WN_001' || _vm.boardMstId == 'BD_WN_002'
          || _vm.boardMstId == 'BD_NT_001' || _vm.boardMstId == 'BD_NT_002'
      ){
        _vm.allowedFileExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'pdf', 'ppt', 'pptx', 'doc', 'docx'];
      } else {
        _vm.allowedFileExtensions = [];
      };
    },
    ...
    async uploadImage(file) {
      ...
      formData.append('boardSeq', _vm.detail.boardSeq || 0);
      formData.append(`board_files_0`, file, file.name);
