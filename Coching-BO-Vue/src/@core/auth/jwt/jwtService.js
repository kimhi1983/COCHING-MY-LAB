...
export default class JwtService {
  ...
  async logout(){
    ...
    const logRouteChange = store.state.erns.logRouteChange || {from : null,
      to : null
    };
  ...
  async forceLogout(){
    ...
    const _vueMainVm = window.erns_coching_bo_vue;
  ...
  async onResponseHandler(response){
    ...
    if (resultCode != "0000") {
      ...
      return Promise.reject(new ApiError(resultFailMessage || '알 수 없는 오류가 발생하였습니다.'
        , response
      ));
