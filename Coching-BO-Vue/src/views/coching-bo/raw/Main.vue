<template>
    <div id="coching-bo-raw-main">
      
      <!-- 검색조건 -->
      <b-card no-body>
        <b-card-body class="cm-bo-raw-filter">
          <b-row>
            <b-col cols="12" md="3" class="mb-md-0">
              <b-form-group label="검색조건" label-for="coching-bo-raw-search-option">
                <v-select
                  id="coching-bo-raw-search-option"
            ...
            <b-col cols="12" md="6" class="mb-md-0">
              <b-form-group label="키워드" label-for="coching-bo-raw-search-keyword">
                <b-input-group>
                  <b-form-input 
                    id="coching-bo-raw-search-keyword"
      ...
      <b-card no-body class="mb-0">
        ...
        >
          ...
          <template #cell(rgtDate)="data">
            {{ data.item.rgtDate  | eFmtDate('-')}}
          </template>
        </b-table>
  ...
  import { getList, setRawState } from '@/api/coching-bo/raw/raw';
  
  const DEFAULT_RAW_TABLE_COLUMNS = [
    { key: 'index'      , label: '번호'           , sortable: false , class: 'text-center' },
    { key: 'rawName'     , label: '원료명'         , sortable: false , class: 'text-center'},
    { key: 'ptnName' , label: '원료사'     , sortable: false , class: 'text-center' },
    { key: 'membName' , label: '담당자'     , sortable: false , class: 'text-center' },
    { key: 'useYn'  , label: '사용여부'       , sortable: false , class: 'text-center' },
    { key: 'rgtDttm'  , label: '등록일'       , sortable: false , class: 'text-center' },
  ];
  ...
  export default {
    ...
    data() {
      ...
      const AG_GRID_COLUMN_DEFS = [
        ...
        { headerName: "원료명", field: "rawName", width: 250, cellClass: 'flex-left' },
        { headerName: "원료사", field: "ptnName", width: 120, cellClass: 'flex-center' },
        { headerName: "담당자", field: "membName", width: 120, cellClass: 'flex-center' },
        { headerName: "성분", field: "ingdNames", width: 250, cellClass: 'flex-left selectable-text',
          valueGetter: (params) => {
            const ingdCnt = params.node.data.ingdCnt;
            const ingdNames = params.node.data.ingdNames;
        ...
        { headerName: "제조사", field: "prodCompany", width: 120, cellClass: 'flex-center' },
        { headerName: "제조국", field: "prodCountry", width: 100, cellClass: 'flex-center' },
        { headerName: "공급사", field: "supplier", width: 150, cellClass: 'flex-center' },
        ...
        { headerName: "inci", field: "ingdNamesEn", width: 250, cellClass: 'flex-left selectable-text',
          wrapText: true, // 자동 줄바꿈 활성화
          autoHeight: true, // 행 높이를 자동 조정
          cellStyle: { "white-space": "normal" } // 여러 줄로 표시
        },

        { headerName: "등록일"        , field: "rgtDttm", width: 180    , cellClass: 'flex-center'},
        { headerName: "최종수정일"        , field: "chngDttm", width: 180    , cellClass: 'flex-center'},
      ...
      return {
        ...
        options: [
          ...
          { value: 'rawName', label: '원료이름' },
          { value: 'ptnName', label: '회사이름' },
          { value: 'rawMemb', label: '담당자이름' },
        ],
    ...
    methods: {
      ...
      onCellClicked(event){
        ...
        //상세이동
        const goDetailFileds = ["rawName", "ptnName", "membName", "prodCompany", "prodCountry", "supplier", "rgtDttm", "chngDttm"];
        if(goDetailFileds.includes(clickField)){
          _vm.$router.push({ name: 'coching-bo-raw-detail', query: {rawSeq: item.rawSeq, rawDetailSeq: item.rawDetailSeq}});
          return;
        }
      },
      ...
      getInitParam() {
        ...
        if(data && data.initParam){
          ...
          } else if (selected.rawName && selected.rawName !== "") {
            _vm.selectedOption = "rawName";
          } else if (selected.ptnName && selected.ptnName !== "") {
            _vm.selectedOption = "ptnName";
          } else if (selected.rawMemb && selected.rawMemb !== "") {
            _vm.selectedOption = "rawMemb";
          } else {
            _vm.selectedOption = "allSrch";
          }
        }
      ...
      onSearch() {
        ...
        } else if(_vm.selectedOption === "rawName") {
          searchOp.rawName = searchOp.srchData;
        } else if(_vm.selectedOption === "ptnName") {
          searchOp.ptnName = searchOp.srchData;
        } else if(_vm.selectedOption === "rawMemb") {
          searchOp.rawMemb = searchOp.srchData;
        }
      ...
      async onClickMultpleSave(){
        ...
        const result = await _vm.$swal({
          title: '확인',
          text: `${params.list.length}개 원료 설정을 저장 하시겠습니까?`,
        ...
        try {        
          ...
          if(result.value){
            await setRawState(params);
            await _vm.onSearch();
            _vm.alertSuccess("원료 설정 값이 변경되었습니다.");
          }
        } catch (err) {
          _vm.alertError(err);
        } finally {
      ...
      async onApplyValue(gridParams){
        ...
        const params = {
          isRegMode: (gridParams.data.rawSeq || 0) <= 0,
          rawSeq : gridParams.data.rawSeq || 0,
        ...
        const result = await _vm.$swal({
          title: '확인',
          text: `원료 상태를 ${params.isRegMode ? '등록': '변경'} 하시겠습니까?`,
          showCancelButton: true,
        ...
        try {        
          ...
          if(result.value){
            await setRawState({list:[params]});
            await _vm.onSearch();
            _vm.alertSuccess("원료 상태가 변경되었습니다.");
          }
        } catch (err) {
          _vm.alertError(err);
        } finally {
      ...
      getListSearchParam() {
        ...
        return {
          ...
          rawNameL: searchOp.rawName,
          ptnNameL : searchOp.ptnName,
          membNameL : searchOp.rawMemb,

          incIngNameYn : 'Y', //성분정보 포함
        }
      },
