  ...
  <div>
      ...
      <b-card-body class="cm-main-filter">
        <b-row>
          <b-col cols="12" md="3"
            class="mb-md-0"
          >
            <b-form-group label="검색조건" label-for="sys-banner-master-search-option">
              <v-select
                id="sys-banner-master-search-option"
          ...
          <b-col cols="12" md="6">
            <b-form-group label="키워드" label-for="sys-banner-master-search-keyword">
              <b-input-group>
    ...
    <b-card no-body class="mb-0">
      ...
      <b-table
        ...
        :items="vBanner.rawList"
        :fields="vBanner.fields"
        ref="vBannerlistTable"
        hover
        primary-key="bannerMstId"
        show-empty
      ...
      <div class="mx-2 mb-2">
        <b-row>
          ...
          >
            <span class="text-muted">
              {{eumPaginationRangeForPageInfo(vBanner.pi)}}
            </span>
          </b-col>
          ...
          >
            <b-pagination
              v-model="vBanner.pi.curPage"
              :total-rows="vBanner.pi.totalItem"
              :per-page="vBanner.pi.perPage"
              first-number
              last-number
              limit = "10"
              class="mb-0 mt-1 mt-sm-0"
...
import {getBannerMasterList, setBannerMasterState} from '@/api/coching-bo/system/bannerMaster';
...
import { DEF_SEARCH_OPT } from '@/constants/banner';
...
export default {
  name: 'coching-BackOffice-System-Main-Banner',
  ...
  data(){
    ...
    const AG_GRID_COLUMN_DEFS = [
      ...
      { headerName: "분류"        , field: "bannerType", width: 100    , cellClass: 'flex-center'},
      { headerName: "코드"        , field: "bannerMstCd", width: 100    , cellClass: 'flex-center'},

      //{ headerName: "설명"        , field: "popupMstDesc", width: 350    , cellClass: 'flex-center'},
      { headerName: "설명" , field: "bannerMstDesc"   , width: 400
        , editable: true
    ...
    return {
     ...
     selectedOption: 'allSrch',
      options: [
        ...
        { value: 'bannerType', label: '분류' },
        { value: 'bannerMstCd', label: '코드' },
        { value: 'bannerMstDesc', label: '설명' },
      ],
  ...
  methods: {
    ...
    getInitParam() {
      ...
      if(data && data.initParam){
        ...
        } else if (selected.bannerType && selected.bannerType !== "") {
          _vm.selectedOption = "bannerType";
        } else if (selected.bannerMstCd && selected.bannerMstCd !== "") {
          _vm.selectedOption = "bannerMstCd";
        } else if (selected.bannerMstDesc && selected.bannerMstDesc !== "") {
          _vm.selectedOption = "bannerMstDesc";
        } else {
          _vm.selectedOption = "allSrch";
        }
    ...
    onCellClicked(event){
      ...
      if(goDetailFileds.includes(clickField)){
        _vm.$router.push({ name: 'erns-coching-bo-member-editForm', query: {membSeq: item.membSeq}});
        return;
      }
    },
    ...
    onCellValueChanged(params) {
      ...
      //Cell 랜더러에게 변경 noti
      const checkFields = ["_isEditRow", "bannerMstDesc"];
    ...
    onSearch() {
      ...
      searchOp.allSrch = "";
      searchOp.bannerType = "";
      searchOp.bannerMstCd = "";
      searchOp.bannerMstDesc = "";

      if(_vm.selectedOption === "allSrch") {
        searchOp.allSrch = searchOp.srchData;
      } else if(_vm.selectedOption === "bannerType") {
        searchOp.bannerType = searchOp.srchData;
      } else if(_vm.selectedOption === "bannerMstCd") {
        searchOp.bannerMstCd = searchOp.srchData;
      } else if(_vm.selectedOption === "bannerMstDesc") {
        searchOp.bannerMstDesc = searchOp.srchData;
      }
    ...
    async onClickMultpleSave(){
      ...
      Object.entries(editCells).forEach(([rowId, editCellInfo]) => {
        ...
        params.list.push({
          bannerMstCd : rowNode.data.bannerMstCd || 0,
          bannerMstDesc : rowNode.data.bannerMstDesc,
      ...
      const result = await _vm.$swal({
        title: '확인',
        text: `${params.list.length}개 배너 마스터 설정을 저장 하시겠습니까?`,
      ...
      try {        
        ...
        if(result.value){
          await setBannerMasterState(params);
          await _vm.onSearch();
    ...
    async onApplyValue(gridParams){
      ...
      const params = {
        isRegMode: (gridParams.data.bannerMstCd || 0) <= 0,
        bannerMstCd : gridParams.data.bannerMstCd || 0,
        bannerMstDesc : gridParams.data.bannerMstDesc,
        useYn: (gridParams.data.useYn || '').trim(),     
      };

      //Validation
      if(!params.bannerMstCd){
        return;
      }

      const result = await _vm.$swal({
        title: '확인',
        text: `배너 마스터 상태를 ${params.isRegMode ? '등록': '변경'} 하시겠습니까?`,
        showCancelButton: true,
      ...
      try {        
        ...
        if(result.value){
          await setBannerMasterState({list:[params]});
          await _vm.onSearch();
          _vm.alertSuccess("배너 마스터 상태가 변경되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
    ...
    async apiList(pageNo){
      ...
      try {
        ...
        //데이터 로드
        const res = await getBannerMasterList(params); 
    ...
    getListSearchParam() {
      ...
      return {
        ...
        bannerTypeL : searchOp.bannerType,
        bannerMstCdL : searchOp.bannerMstCd,
        bannerMstDescL : searchOp.bannerMstDesc,
      }
    },
