<template>
  <div>
    <!-- 검색조건 -->
    <b-card no-body>
      <b-card-body class="sys-code-master-filter">
        <b-row>
          <b-col
            cols="12" md="6">
            <b-form-group
              label="검색" label-for="sys-code-master-search-text">
              <b-input-group>
                <b-form-input 
                  id="sys-code-master-search-text"
                  v-model="listData.sc.searchText"
                  @keyup.enter="loadList(1)"
                  placeholder="그룹코드, 그룹코드명을 검색하세요."
                  size="sm"
                ></b-form-input>
            ...
            <div class="d-flex align-items-center justify-content-end">
              <b-button
                variant="primary"
                 @click="insertCodeMst"
              >
                <span class="text-nowrap">신규등록</span>
              </b-button>
            </div>  
          </b-col> -->
...
<script>
import {getCodeMasters, setCodeMasterState} from '@/api/coching-bo/comm/code';
...
export default {
  name: 'coching-BackOffice-System-Code-Master',
  ...
  data(){
    ...
    const AG_GRID_COLUMN_DEFS = [
      ...
      { headerName: "그룹코드명"  , field: "grpCodeName", width: 150    , cellClass: 'flex-left'},
      { headerName: "설명"        , field: "codeDesc", width: 250    , cellClass: 'flex-left'},
      { headerName: "기타 코드설명", field: "etcCmmt", width: 250    , cellClass: 'flex-left'},
  ...
  methods: {
    ...
    onCellClicked(event){
      ...
      const goDetailFileds = ["grpCode", "grpCodeName", "codeDesc", "etcCmmt", "chngDttm"];
      if(goDetailFileds.includes(clickField)){
        _vm.$router.push({ name: 'coching-bo-system-code-detail', query: { grpCode: item.grpCode}});
        return;
      }
    },    
    ...
    async onClickMultpleSave(){
      ...
      const result = await _vm.$swal({
        title: '확인',
        text: `${params.list.length}개 그룹코드 설정을 저장 하시겠습니까?`,
      ...
      try {        
        ...
        if(result.value){
          await setCodeMasterState(params);
          await _vm.onSearch();
          _vm.alertSuccess("그룹코드 설정 값이 변경되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
    ...
    async onApplyValue(gridParams){
      ...
      const params = {
        isRegMode: (gridParams.data.grpCode || 0) <= 0,
      ...
      const result = await _vm.$swal({
        title: '확인',
        text: `그룹코드 상태를 ${params.isRegMode ? '등록': '변경'} 하시겠습니까?`,
        showCancelButton: true,
      ...
      try {        
        ...
        if(result.value){
          await setCodeMasterState({list:[params]});
          await _vm.onSearch();
          _vm.alertSuccess("그룹코드 상태가 변경되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
    ...
    async loadList(pageNo){
      ...
      try {
        ...
        //데이터 로드
        const res = await getCodeMasters(params); 
