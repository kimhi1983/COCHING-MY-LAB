    ...
    <b-card no-body>
      <b-card-body>
        <b-row>
          ...
          <b-col cols="12" md="6">
            <b-form-group label="제목" label-for="search-coching-tv-keyword-searchText">
              <b-input-group>
                <b-form-input
                  id="search-coching-tv-keyword-searchText"
                  v-model="listData.sc.searchText"
                  @keyup.enter="onSearch"
                  placeholder="제목를 검색하세요."
                  size="sm"
                ></b-form-input>                
            ...
            <div class="erns-filter-div mt-1 text-right">
              <b-form-group label="">
                ...
                <b-button
                  ...
                  ><feather-icon icon="RefreshCcwIcon" size="14" /> T_ES_TV 초기화
                </b-button>
...
export default {
  ...
  data() {
    ...
    const AG_GRID_COLUMN_DEFS = [
      ...
      { headerName: "아이디"        , field: "tvSeq"        , width: 70  , cellClass: 'flex-right',
        wrapText: true, // 자동 줄바꿈 활성화
        autoHeight: true, // 행 높이를 자동 조정
        cellStyle: { "white-space": "normal" } // 여러 줄로 표시
      },
      ...
      { headerName: "조회수"      , field: "views" , width: 120  , cellClass: 'flex-right',
        wrapText: true, // 자동 줄바꿈 활성화
        autoHeight: true, // 행 높이를 자동 조정
        cellStyle: { "white-space": "normal" } // 여러 줄로 표시
      },
  ...
  methods: {
    ...
    async onClickResetTable() {
      ...
      try {        
        ...
        if(result.value){
          await resetEsCochingTvTable(params);
          await _vm.onSearch();
          _vm.alertSuccess("T_ES_TV 테이블이 초기화 되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
    ...
    async onClickTransferToEs() {
      ...
      try {
        ...
        if(result.value){
          await getEsCochingTvCreateEsIndex(params);
          await _vm.loadIndexList();
    ...
    async loadList(pageNo) {
      ...
      try {
        ...
        //데이터 로드
        const res = await getEsCochingTvManageList(params);
