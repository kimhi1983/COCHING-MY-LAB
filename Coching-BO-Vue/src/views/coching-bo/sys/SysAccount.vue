<template>
  <div id="coching-BackOffice-System-Management-Account-Main">
    <b-card no-body>
      <b-card-body class="cm-filter">
        <b-row>
          <b-col cols="12" md="3" class="mb-md-0">
            <b-form-group label="아이디" label-for="sys-account-membIdL">
              <b-input-group>
                <b-form-input 
                  id="sys-account-membIdL"
                  v-model="listData.sc.membIdL"
          ...
          <b-col cols="12" md="3" class="mb-md-0">
            <b-form-group label="성명" label-for="sys-account-membNameL">
              <b-input-group>
                  <b-form-input 
                    id="sys-account-membNameL"
                    v-model="listData.sc.membNameL"
                    @keyup.enter="onSearch"
          ...
          <b-col cols="12" md="3" class="mb-md-0">
            <b-form-group label="연락처" label-for="sys-account-phoneL">
              <b-input-group>
                <b-form-input 
                  id="sys-account-phoneL"
                  v-model="listData.sc.phoneL"
                  @keyup.enter="onSearch"
...
import { DEF_SEARCH_OPT } from '@/constants/account';
...
import { getAdminMembList, setAdminState } from '@/api/coching-bo/user/adminUser'
...
export default {
  ...
  data(){
    ...
    const AG_GRID_COLUMN_DEFS = [
      ...
      { headerName: "아이디"        , field: "membId", width: 120    , cellClass: 'flex-center'},
      { headerName: "성명"          , field: "membName", width: 150  , cellClass: 'flex-center'},
      { headerName: "연락처"        , field: "phone", width: 120     , cellClass: 'flex-center'
        , valueFormatter: (params) => params.value ? this.$options.filters.eufmtPhoneDash(params.value) : '-'
      },
      { headerName: "e-mail"        , field: "email", width: 250     , cellClass: 'flex-center'},
      ...
      { headerName: "가입일"        , field: "rgtDttm", width: 150   , cellClass: 'flex-center'},
  ...
  methods: {
    ...
    getInitParam() {
      ...
      console.info("getInitParam", data);
    ...
    onCellClicked(event){
      ...
      //상세이동
      const goDetailFileds = ["membId", "membName", "phone", "email", "rgtDttm"];
      if(goDetailFileds.includes(clickField)){
        _vm.$router.push({ name: 'coching-bo-sys-account-edit', query: {membSeq: item.membSeq}});
        return;
      }
    },
    ...
    async onClickMultpleSave(){
      ...
      const result = await _vm.$swal({
        title: '확인',
        text: `${params.list.length}개 계정 설정을 저장 하시겠습니까?`,
      ...
      try {        
        ...
        if(result.value){
          await setAdminState(params);
          await _vm.onSearch();
          _vm.alertSuccess("계정 설정 값이 변경되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
    ...
    async onApplyValue(gridParams){
      ...
      const result = await _vm.$swal({
        title: '확인',
        text: `계정 상태를 ${params.isRegMode ? '등록': '변경'} 하시겠습니까?`,
        showCancelButton: true,
      ...
      try {        
        ...
        if(result.value){
          await setAdminState({list:[params]});
          await _vm.onSearch();
          _vm.alertSuccess("계정 상태가 변경되었습니다.");
        }
      } catch (err) {
        _vm.alertError(err);
      } finally {
    ...
    async loadList(pageNo){
      ...
      try {
        ...
        //데이터 로드
        const res = await getAdminMembList(params); 
    ...
    onClickNew() {
      const _vm = this;
       _vm.$router.push({ name: 'coching-bo-sys-account-add', query: {membSeq: 0}});
    }
  }  
}
</script>
