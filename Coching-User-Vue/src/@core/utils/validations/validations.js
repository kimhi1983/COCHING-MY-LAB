...
export const cochingValidation = vm => {
  ...
  extend('cochingUserId', {
    validate: value => {
      return validatorCochingUserId(value) ? true : i18n.t('') || '{_field_}는 영문자 포함 6자이상이어야 합니다.';
    }
  });
  ...
  extend('engHanNumber3', {
    validate: value => {
      return validatorEngHanNumber3(value) ? true : i18n.t('', { _field_: `{_field_}` }) || '{_field_}은(는) 공백없이 영문,한글,숫자만 3자이상 입력해 주십시오.';
    }
  });
