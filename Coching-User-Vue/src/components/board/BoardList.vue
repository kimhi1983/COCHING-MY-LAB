  ...
  <div>
    ...
    <div class="total-wrap">
      ...
      <div class="right">
        ...
        <!--button-->
        <button 
          v-if="boardMstId == 'BD_RAW_001' || boardMstId == 'BD_RAW_002'"
      ...
      <a href="javascript:;"
       ...
       @click="onClickItem(item)">
        <div class="content-wrap">
          ...
          <span v-if="item.cmtUseYn == 'Y' && item.cmtCnt"
            class="text-primary">({{item.cmtCnt}})</span>
          <div v-if="isNew(item)" class="new"></div>
          </div>
          <div class="date">
            <span v-if="boardMstId == 'BD_NT_001' || boardMstId == 'BD_FAQ_001'">관리자</span>
            <span v-else>{{item.regName}}</span>
...
export default {
  ...
  methods : {
    ...
    isNew(item){
      return moment().diff(moment(item.rgtDttm), 'days') <= 3;
    },
