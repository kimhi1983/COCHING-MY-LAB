...
export default {
  ...
  methods: {
    async docReady(){
      ...
      const action = window.$giCochingApp.getServerSideAutoAction();
      const message = window.$giCochingApp.getServerSideAutoMessage();
      const loginInfo = window.$giCochingApp.getServerSideAutoLoginRet();
      const joinInfo = window.$giCochingApp.getServerSideAutoJoinRet();
      ...
      switch(action){
        case "leaveUser":          
          ...
          //Move login
          _vm.ermReplacePage({name:'coching-login', params:{
            joinErrorCode: "20901",
