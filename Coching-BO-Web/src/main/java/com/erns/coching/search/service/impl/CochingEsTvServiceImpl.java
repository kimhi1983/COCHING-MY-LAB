...
import com.erns.coching.search.mapper.CochingEsTvDAO;
...
@Transactional
@Service
public class CochingEsTvServiceImpl extends AbstractCochingApiService implements CochingEsTvService {
  ...
  private static final DateTimeFormatter indexNameTimeFormatter = DateTimeFormatter.ofPattern("yyyyMMddHHmmss");
	...
	@Autowired
	public CochingEsTvServiceImpl(RestTemplateBuilder builder, CochingEsTvDAO dao) {
		super();
	...
	public boolean resetCochingTvIndex(){
		...
		{	//새로 생성할 인덱스 명 생성			
			if(null == indexPrefixProp){
				log.error("시스템설정 키 : {} 존재하지 않습니다.", Const.ES_COCHING_TV_INDEX_PREFIX_SYSKEY);
				return false;
			}
		...
		{	//active properties, inactive properties 를 제외한 인덱스 삭제
			SysPropVO activeProp = sysPropService.loadVo(Const.ES_COCHING_TV_ACTIVE_INDEX_SYSKEY);
			SysPropVO inactiveProp = sysPropService.loadVo(Const.ES_COCHING_TV_INACTIVE_INDEX_SYSKEY);
	...
	public boolean createEsIndex(String indexName) throws JsonProcessingException {
		...
		{	//active properties => inactive properties
			...
			if(null != activeProp){				
				SysPropVO inactiveProp = SysPropVO.builder()
						.sysKey(Const.ES_COCHING_TV_INACTIVE_INDEX_SYSKEY)
		...
		// 인덱스 생성
		String filePath = String.format("classpath:/es/mapping/coching_tv.json");
		...
		{	//set new active properties
			SysPropVO activeProp = SysPropVO.builder()
					.sysKey(Const.ES_COCHING_TV_ACTIVE_INDEX_SYSKEY)
					.sysValue(indexName)
					.build();
	...
	public int bulkInsertToEs(String indexName, int chunkSize) {
		CochingEsTvSearchDTO param = new CochingEsTvSearchDTO();
		...
		for (int page = 0; page < pageCount; page++) {
			...
			List<CochingEsTvVO> chunkData = getListVo(param);
			...
			log.info("bulkInsertToEs resultCode:{}, messge:{}", apiResult.getResultCode(), apiResult.getResultMessage());
		}
	...
	public int bulkSyncDocToEs(String indexName, long[] tvSeqs) {
		if(tvSeqs == null || tvSeqs.length == 0){
			return 0;
		}

		CochingEsTvSearchDTO param = new CochingEsTvSearchDTO();
		param.setRowSize(-1);
		param.setTvSeqs(tvSeqs);

		//index 대상
		List<CochingEsTvVO> esDocList = dao.getListVoForEsRenewIndexItems(param);

		//delete 대상
		List<CochingEsTvVO> esDocDelList = dao.getListVoForEsRenewDeleteItems(param);
	 ...
	 * @return
	 */
	private String convertToNDJSON(List<CochingEsTvVO> dataList, String indexName) {
		StringBuilder ndJsonBuilder = new StringBuilder();

		for (CochingEsTvVO data : dataList) {
			String id = String.valueOf(data.getTvSeq());
